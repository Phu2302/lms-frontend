...
function ConfirmStudent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Error fetching confirm student data:', err);
  ...
  // Filter requests for cert/confirm (excludes card & course withdrawal)
  const certFilteredRequests = requests.filter(r => {
    const reasonText = r.noiDung || r.reason || '';
    return !reasonText.startsWith('[In thẻ SV]') && !reasonText.startsWith('[Rút môn học]');
  });

  const pendingRequests = certFilteredRequests.filter(r => r.tinhTrang === 'Đang xử lý' || r.status === 'PENDING' || r.status === 'Đang xử lý');
  const completedRequests = certFilteredRequests.filter(r => r.tinhTrang === 'Đã xử lý' || r.status === 'COMPLETED' || r.status === 'Đã xử lý');
  ...
  const displayInfo = {
    ...
    ngayCap: studentData?.ngayCap || 'N/A',
    noiCap: studentData?.noiCap || 'N/A',
  };
      ...
      <section className="service-section">
        ...
        <form className="service-form" onSubmit={handleRegister}>
          <div className="form-group">
            ...
            <input 
              type="text" 
              value="Xác nhận sinh viên - Học kỳ 3 Năm học 2025 - 2026" 
              disabled 
